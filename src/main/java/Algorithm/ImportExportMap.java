...
public class ImportExportMap {
     ...
     */
    public static void exportBinary(VoronoiMap map, String fileName) throws IOException {
        if (map == null) {
    ...
    public static VoronoiMap importBinary(String fileName)
        ...
        try (ObjectInputStream ois = new ObjectInputStream(
            ...
            throw new IOException("File is corrupted or not a binary map: " + filePath, e);
        } catch (InvalidClassException e) {
            throw new IOException("File was created with an older version. Please export again.", e);
        }
    }

    /**
     * Imports hospitals from a CSV file in the data folder.
     * Expected format per line: {@code name,x,y,maxCapacity}.
     * Lines starting with {@code #} are treated as comments and skipped.
     *
     * @param fileName the file name, for example {@code hospitals.csv}
     * @param map the map to populate
     * @return the number of hospitals successfully imported
     * @throws IllegalArgumentException if fileName or map is invalid
     * @throws IOException if the file cannot be read
     */
    public static int importHospitalsCSV(String fileName, VoronoiMap map) throws IOException {
        if (map == null) {
            throw new IllegalArgumentException("Map cannot be null");
        }
